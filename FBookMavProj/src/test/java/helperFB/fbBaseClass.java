...
import DataProviderFactory.FBdataProviderFactory;
...
public class fbBaseClass {
	...
	//we are creating obj so that we can reference back them
		...
		public void setupReport()
		{
			...
			ExtentHtmlReporter reporter= new ExtentHtmlReporter(new File(System.getProperty("user.dir")+ "/Reports/AP_ "+UtilityFB.getTime()+".html"));
		...
		public void setupBrowser(){
			// we are calling the  driver first from browser fctory
						...
						driver =fbBrowserFactory.startApplication(
								FBdataProviderFactory.getConfig().getBrowser()
								,FBdataProviderFactory.getConfig().getStagingURL());
		...
		public void appendReport(ITestResult result){
			...
			{
				try{
					//trying to catch the error MediaEntityModelProvider.. 
					...
					logger.pass("Test Scenario pass",MediaEntityBuilder.createScreenCaptureFromPath(UtilityFB.captureScreenshot(driver)).build());
			...
			else if(status==ITestResult.FAILURE)
			{
				...
				logger.fail("Test Failed"+result.getThrowable().getMessage(),MediaEntityBuilder.createScreenCaptureFromPath(UtilityFB.captureScreenshot(driver)).build());
			}catch(IOException e){
				System.out.println("Not able to attach screenshot"+ e.getMessage());
				}
